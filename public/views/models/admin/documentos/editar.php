...
if (isset($_GET['id_tipdocu']) && is_numeric($_GET['id_tipdocu']) && $_GET['id_tipdocu'] > 0) {
    $id_docu = $_GET['id_tipdocu'];
    $stmt = $conexion->prepare("SELECT * FROM tipdocu WHERE id_tipdocu = :id_tipdocu");
    $stmt->bindParam(':id_tipdocu', $id_docu);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($result) {
        $old_docu = $result['tipdocu'];
    } else {
        echo '<script> alert ("ID de Documento no válido");</script>';
        echo '<script> window.location="index.php"</script>';
    }
}

if ((isset($_POST["registro"])) && ($_POST["registro"] == "formu")) {
    $docu = $_POST['id_tipdocu'];

    $validar = $conexion->prepare("SELECT * FROM tipdocu WHERE tipdocu = :docu");
    $validar->bindParam(':docu', $docu);
    $validar->execute();
    ...
    if (empty($docu)) {
        ...
        echo '<script> alert ("El T.P Documento ya existe");</script>';
        echo '<script> window.location="index.php"</script>';
    } else {
        // Actualización
        $updatesql = $conexion->prepare("UPDATE tipdocu SET tipdocu = :docu WHERE id_tipdocu = :id_docu");
        $updatesql->bindParam(':docu', $docu);
        $updatesql->bindParam(':id_docu', $id_docu);
        $updatesql->execute();
            ...
            <div class="dlabnav-scroll">
                <ul class="metismenu" id="menu">
                    ...
                    <li><a class="has-arrow " href="javascript:void()" aria-expanded="false">
                        ...
                        <ul aria-expanded="false">
                            <li><a href="./index.php">Lista Documentos</a></li>
                            <li><a href="./crear.php">Crear Documentos</a></li>
                        </ul>
                    </li>
                    <!-- MODULO DE EMBALAJE -->
                    <li><a class="has-arrow " href="javascript:void()" aria-expanded="false">
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h2>Editar T.P Documento</h2>
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="modal-body">
                                    <label for="id_tipdocu">Nombre Del Documento</label>
                                    <input id="id_tipdocu" type="text" class="form-control" name="id_tipdocu" value="<?php echo $old_docu; ?>" placeholder="Ingresa un T.P documento" required>
                                </div>

                                <div class="row" style="font-size: 25px;">
                                        <button type="submit" name="registro" value="formu" class="btn_ing btn btn-margin col-4 mx-auto" style="background-color: #0097B2; color: #ffff;">EDITAR T.P DOCUEMNTO</button>
                                </div>
                            </form>
                                </div>
                            </div>
