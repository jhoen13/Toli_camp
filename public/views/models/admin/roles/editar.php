...
if (isset($_GET['id_rol']) && is_numeric($_GET['id_rol']) && $_GET['id_rol'] > 0) {
    $id_rol = $_GET['id_rol'];
    $stmt = $conexion->prepare("SELECT * FROM roles WHERE id_rol = :id_rol");
    $stmt->bindParam(':id_rol', $id_rol);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($result) {
        $old_rol = $result['tipo_rol'];
    } else {
        echo '<script> alert ("ID de Rol no válido");</script>';
...
if ((isset($_POST["registro"])) && ($_POST["registro"] == "formu")) {
    ...
    $validar = $conexion->prepare("SELECT * FROM roles WHERE tipo_rol = :rol");
    $validar->bindParam(':rol', $rol);
    $validar->execute();
    ...
    if (empty($rol)) {
        ...
        // Actualización
        $updatesql = $conexion->prepare("UPDATE roles SET tipo_rol = :rol WHERE id_rol = :id_rol");
        $updatesql->bindParam(':rol', $rol);
        $updatesql->bindParam(':id_rol', $id_rol);
        $updatesql->execute();
            ...
            <div class="dlabnav-scroll">
                <ul class="metismenu" id="menu">
                    ...
                    <li>
                        ...
                        <ul aria-expanded="false">
                            <li><a href="./index.php">Lista Roles</a></li>
                            <li><a href="./crear.php">Crear Roles</a></li>
                        </ul>
                    </li>
                            ...
                            <div class="card-body">
                                ...
                                <form action="" method="post">
                                    <div class="modal-body">
                                        ...
                                        <input id="rol" type="text" class="form-control" name="rol" value="<?php echo $old_rol; ?>" placeholder="Ingresa un genero" required>
                                    </div>
                            
                                    <div class="row" style="font-size: 25px;">
                                            <button type="submit" name="registro" value="formu" class="btn_ing btn btn-margin col-4 mx-auto" style="background-color: #0097B2; color: #ffff;">EDITAR USUARIO</button>
                                    </div>
                                </form>
                            </div>
                            </div>
...
if ((isset($_POST["registro"])) && ($_POST["registro"] == "formu")) {
    ...
    $validar = $conexion->prepare("SELECT * FROM roles WHERE tip_rol = :rol");
    $validar->bindParam(':rol', $rol);
    $validar->execute();
    $filaa1 = $validar->fetchAll(PDO::FETCH_ASSOC);
    ...
    if (empty($rol)) {
        ...
        // Actualización
        $updatesql = $conexion->prepare("UPDATE roles SET tip_rol = :rol WHERE id_rol = :id_rol");
        $updatesql->bindParam(':rol', $rol);
        $updatesql->bindParam(':id_rol', $id_rol);
        $updatesql->execute();
...
<head>
    ...
    <title>Formulario de actualización de Rol</title>
</head>
<body>
