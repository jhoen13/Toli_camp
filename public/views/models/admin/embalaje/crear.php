...
if ((isset($_POST["registro"])) && ($_POST["registro"] == "formu")) {
    ...
    // Verificar si el embalaje ya existe
    $validar = $conexion->prepare("SELECT * FROM embalaje WHERE embalaje = :embalaje");
    ...
    if ($embalaje == "") {
        ...
        // Si el embalaje ya existe, muestra un mensaje de error
        echo '<script>alert("El embalaje ya existe en la base de datos");</script>';
        echo '<script>window.location="index.php"</script>';
    } else {
        // Si el embalaje no existe, realizar la inserción
        $insertsql = $conexion->prepare("INSERT INTO embalaje(embalaje) VALUES (:embalaje)");
        $insertsql->bindParam(':embalaje', $embalaje);
        $insertsql->execute();
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <h2 class="modal-title" id="exampleModalLabel">Crear Embalaje</h2>
                                    <table id="example3" class="display text-center" style="min-width: 845px">
                                        <form action="" method="post" enctype="multipart/form-data"> 
                                            ...
                                            <div class="modal-footer">
                                                <button type="submit" name="registro" value="formu" class="btn_ing btn btn-margin col-4 mx-auto" style="background-color: #0097B2; color: #ffff;">CREAR EMBALAJE</button>    
                                            </div>
                                        </form>
                                    </table>
                                </div>
